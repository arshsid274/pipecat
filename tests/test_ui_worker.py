...
class _FakeClassifier:
    ...
    async def choice(self, state, options, criteria):
        from pipecat.classifiers.base_classifier import ChoiceResult

        self.asked.append((state, options, criteria))
...
class TestBaseUIWorkerScreen(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_which_element_asks_over_the_named_elements(self):
        ...
        state, options, criteria = classifier.asked[0]
        self.assertEqual(state["utterance"], "the Taylor Swift one")
        self.assertIn("<ui_state>", state["screen"])
        self.assertEqual(options["e5"], 'button "Bad Bunny"')
        self.assertEqual(options["e6"], 'button "Taylor Swift"')
        self.assertNotIn("e1", options)
        self.assertIn("referring to", criteria)
