...
#: The three answers and what each one looks like.
    ...
    async def _classify(self):
        ...
        try:
            result = await asyncio.wait_for(
                self._classifier.choice(state, TURN_COMPLETION_OPTIONS, TURN_COMPLETION_CRITERIA),
        ...
        logger.debug(f"{self}: {result.label} ({result.confidence:.2f}) for [{self._text}]")
        if result.label == "short":
            self._timeout_task = self.task_manager.create_task(
                self._timeout(self._short_timeout), f"{self}::_short_timeout"
            )
        elif result.label == "long":
            self._timeout_task = self.task_manager.create_task(
