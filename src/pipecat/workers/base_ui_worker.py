...
class BaseUIWorker(BaseWorker):
    ...
    async def which_element(self, description: str, threshold: float = 0.5) -> str | None:
        ...
        result = await self._classifier.choice(
            state, options, "the element on screen the user is referring to"
        )
        logger.debug(f"{self.name}: '{description}' -> {result.label} ({result.confidence:.2f})")
        if result.confidence < threshold:
            return None
        return result.label

    def _element_options(self) -> dict[str, str]:
